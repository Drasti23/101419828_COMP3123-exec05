...
- By using express.Router() , it allows us to create modular route handler and the router is made for the organistaion of the different path such we used in this app  for example: , the home.js , login,logout.js and profile.js making this it is more compatible and maintable.

benefits are it provides modular organisation system, middleware support to handle the requests that belong to a specific route group.
...
- Express provides a built-in way to handle errors that occur in route handlers or other middleware by defining an error-handling middleware function.
for eg.:
app.use((err, req, res, next) => {
    console.error(err.stack); // Log the error stack for debugging
    res.status(500).send('Server Error'); // Send a generic server error response
});
If any error occurs in the application, it will be caught by this middleware in this example
...
- This line shows where the app should be running on like which port it should be run on. Which improves in flexibility of the environment.
